...
- **Permission-gated** (`requirePermission`): `sales.write` for `create-from-cart`, `pos.operate` for the register list/detail and gift-card activation, `pos.till.manage` to open a till, `pos.till.adjust` to clear a register block. `sales.read` ("View orders") for till detail (`tills/[id].ts`) and running totals (`tills/[id]/summary.ts`): it is the key of both till pages, so granting a role the till screens in Roles also grants their data. The till list (`tills/index.ts`) serves the Till screen and the Till Reconciliation report, so it takes `{ anyOf: ["sales.read", "reporting.read"] }`. The POS does not use it: it reads its register's open till as `openTillId` from `GET /api/registers/[id]` (`pos.operate`), so "Operate register" alone does not open every till's history.
...
- **Session-only** (any signed-in user): `pages/api/print/order/[id].ts` (bare `getServerSession`), plus `pages/api/gift-cards/lookup.ts` and `pages/api/gift-cards/presets/resolve.ts` (`requireAuth`).
