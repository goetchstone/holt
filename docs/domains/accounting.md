...
| `__tests__/integration/generateSalesJournal.integration.test.ts` | A (Postgres) | `generateSalesJournal` end-to-end — 10 scenarios: happy-path balanced JE, B1 cancelled-line filter against real rows, B3 sale-in-reverse signed amounts (unclassified default), B3 mixed-sign per-order, B3 large-dollar precision, **B3 classified `WRITTEN_OFF` Return debits the shrinkage GL** (added 2026-07-24), **B3 classified `RESTOCKED` Return books identically to the default** (added 2026-07-24), idempotency, refusal on POSTED, empty day. Added 2026-04-30 (PR #189, pre-squash repo numbering — historical reference only). |
