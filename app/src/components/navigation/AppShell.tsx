...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-brand-linen">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        {PRODUCT.attribution ? (
          <footer className="px-4 py-4 text-center text-xs text-brand-gray/60">
            <a
              href={PRODUCT.makerUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="transition-colors hover:text-brand-gray"
            >
              {PRODUCT.attribution}
            </a>
          </footer>
