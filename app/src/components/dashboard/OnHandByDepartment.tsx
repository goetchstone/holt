...
export default function OnHandByDepartment() {
  ...
  return (
    <div>
      <h2 className="text-xl text-brand-blue mb-2">Snapshot On-Hand by Department</h2>
      <div className="border border-brand-gray rounded-lg overflow-hidden shadow-sm">
        <table className="min-w-full text-left text-sm whitespace-nowrap">
          <thead className="bg-brand-linen text-brand-black">
            <tr>
              <th className="p-2 border-b border-brand-gray">Department</th>
              <th className="p-2 border-b border-brand-gray text-right">Total On-Hand Quantity</th>
              <th className="p-2 border-b border-brand-gray text-right">Total On-Hand Cost</th>
            </tr>
          </thead>
          <tbody>
            {totals.map((item, i) => (
              <tr key={i} className="odd:bg-white even:bg-brand-stripe">
                <td className="p-2 border-b border-brand-gray">{item.department}</td>
                <td className="p-2 border-b border-brand-gray text-right font-bold">
            ...
            {totals.length === 0 && (
              <tr>
                <td colSpan={3} className="p-4 text-center text-brand-gray">
