...
export function TimeTrackingView({ canSeeAll }: Readonly<{ canSeeAll: boolean }>) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-brand-blue">Time Tracking</h1>

      <section className="mt-5 grid gap-3 rounded-md border border-black/10 bg-brand-linen p-4">
        <form onSubmit={submit} className="grid gap-3 sm:grid-cols-12 sm:items-end">
          <label className="block text-sm sm:col-span-5">
            <span className="mb-1 block font-medium text-brand-black">What did you work on?</span>
            <input
              value={description}
          ...
          <label className="block text-sm sm:col-span-2">
            <span className="mb-1 block font-medium text-brand-black">Time</span>
            <input
              value={duration}
          ...
          <label className="block text-sm sm:col-span-2">
            <span className="mb-1 block font-medium text-brand-black">Date</span>
            <input
              type="date"
              value={date}
          ...
          <label className="flex items-center gap-2 pb-2 text-sm text-brand-gray sm:col-span-2">
            <input
              type="checkbox"
              checked={isBillable}
          ...
          <button
            type="submit"
            disabled={saving}
            className="min-h-[44px] rounded-md bg-brand-navy px-4 text-sm font-medium text-white transition hover:bg-brand-blue disabled:opacity-60 sm:col-span-1"
          >
            {saving ? "…" : "Log"}
          </button>
        </form>
        ...
        <div className="relative max-w-sm text-sm">
          {customer ? (
            <div className="flex items-center gap-2">
              ...
              <span className="rounded bg-white px-2 py-1 text-brand-black">
                  ...
                  {customerMatches.map((c) => (
                    <li key={c.id}>
                      <button
                        ...
                        className="block w-full px-3 py-2 text-left text-sm hover:bg-brand-stripe"
                      >
                        {customerName(c)}
                      </button>
                    </li>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-brand-gray">
          ...
          total · <span className="text-brand-black">{formatMinutes(summary.billableMinutes)}</span>{" "}
          billable ·{" "}
          <span className="text-brand-black">{formatMinutes(summary.unbilledBillableMinutes)}</span>{" "}
          unbilled
        </p>
        {canSeeAll ? (
...
function renderTable(
  ...
  if (entries.length === 0) return <p className="text-brand-gray">No time logged yet.</p>;
  return (
        ...
        <tbody>
          {entries.map((e) => (
            <tr key={e.id} className="border-t border-black/5">
              <td className="px-3 py-2 text-brand-gray">{dateFmt.format(new Date(e.date))}</td>
              <td className="px-3 py-2 text-brand-black">{e.description}</td>
              {showStaff ? (
                <td className="px-3 py-2 text-brand-gray">{e.staffMember.displayName}</td>
              ) : null}
              <td className="px-3 py-2 text-brand-gray">
                {e.customer ? customerName(e.customer) : "—"}
              </td>
              <td className="px-3 py-2 text-brand-black">{formatMinutes(e.minutes)}</td>
              <td className="px-3 py-2">
                {e.isBillable ? (
                  ...
                  <span className="text-xs text-brand-gray">Non-billable</span>
                )}
              </td>
