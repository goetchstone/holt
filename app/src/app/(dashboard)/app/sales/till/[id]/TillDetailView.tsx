  ...
  return (
    <div className="bg-white rounded-lg border border-brand-gray/20 shadow-md p-6">
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.map((c) => (
            <tr key={c.denomination} className="border-b border-brand-gray/5">
              <td className="py-2 text-brand-black">{c.denomination}</td>
              <td className="py-2 text-right text-brand-gray">{c.quantity}</td>
              <td className="py-2 text-right text-brand-black">{fmt(c.amount)}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr className="border-t-2 border-brand-gray/20">
            ...
            <td className="py-3 text-right font-semibold text-brand-black">{fmt(phaseTotal)}</td>
          </tr>
        </tfoot>
      </table>
    </div>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {payments.map((p) => (
            <tr key={p.id} className="border-b border-brand-gray/5">
              <td className="py-2 pr-4 text-brand-gray">
                {new Date(p.paymentDate).toLocaleString()}
              </td>
              <td className="py-2 pr-4 text-brand-black">{p.salesOrder?.orderno || "--"}</td>
              <td className="py-2 pr-4">
                <span className="text-brand-black">{METHOD_LABELS[p.method] || p.method}</span>
                {p.isRefund && (
              ...
              <td
                className={`py-2 text-right font-medium ${p.isRefund ? "text-red-600" : "text-brand-black"}`}
              >
                {p.isRefund ? "-" : ""}
                {fmt(p.paymentAmount)}
              </td>
...
export function TillDetailView({ id }: Readonly<{ id: string }>) {
  ...
  if (loading) {
    return <p className="text-brand-gray font-serif py-4">Loading...</p>;
  }

  if (!till) {
    return <p className="text-brand-gray font-serif py-4">Till not found.</p>;
  }
  ...
  return (
    <div className="py-2 space-y-6 font-serif">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl text-brand-blue font-semibold">{till.register.name}</h1>
          <p className="text-sm text-brand-gray mt-1">{till.register.storeLocation.name}</p>
        </div>
        <span
          className={`text-xs px-2 py-0.5 rounded ${STATUS_STYLES[till.status] || "bg-brand-gray/10 text-brand-gray"}`}
        >
          {till.status}
        </span>
      </div>
      ...
      <div className="bg-white rounded-lg border border-brand-gray/20 shadow-md p-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          <div>
            <p className="text-brand-gray">Opened By</p>
            <p className="text-brand-black font-medium">{till.openedBy.displayName}</p>
          </div>
          <div>
            <p className="text-brand-gray">Opened At</p>
            <p className="text-brand-black">{fmtDate(till.openedAt)}</p>
          </div>
          <div>
            <p className="text-brand-gray">Closed By</p>
            <p className="text-brand-black font-medium">{till.closedBy?.displayName || "--"}</p>
          </div>
          <div>
            <p className="text-brand-gray">Closed At</p>
            <p className="text-brand-black">{fmtDate(till.closedAt)}</p>
          </div>
        </div>
      </div>
      ...
      {/* Financials */}
      <div className="bg-white rounded-lg border border-brand-gray/20 shadow-md p-6">
        <h2 className="text-lg font-semibold text-brand-black mb-4">Financials</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          <div>
            <p className="text-brand-gray">Opening Cash</p>
            <p className="text-brand-black font-medium">{fmt(till.openingCash)}</p>
          </div>
          <div>
            <p className="text-brand-gray">Expected Cash</p>
            <p className="text-brand-black font-medium">{fmt(till.expectedCash)}</p>
          </div>
          <div>
            <p className="text-brand-gray">Actual Cash</p>
            <p className="text-brand-black font-medium">{fmt(till.actualCash)}</p>
          </div>
          <div>
            <p className="text-brand-gray">Variance</p>
      ...
      <div className="bg-white rounded-lg border border-brand-gray/20 shadow-md p-6">
        ...
        {till.payments.length === 0 ? (
          <p className="text-sm text-brand-gray">No payments recorded on this till.</p>
        ) : (
      ...
      {till.notes && (
        <div className="bg-white rounded-lg border border-brand-gray/20 shadow-md p-6">
          <h2 className="text-lg font-semibold text-brand-black mb-2">Notes</h2>
          <p className="text-sm text-brand-gray whitespace-pre-wrap">{till.notes}</p>
        </div>
      )}
        ...
        <div className="bg-white rounded-lg border border-brand-gray/20 shadow-md p-6">
          {!showReconcile ? (
            ...
            <div className="space-y-3">
              <h2 className="text-lg font-semibold text-brand-black">Reconcile Till</h2>
              <div>
                <label htmlFor="reconcile-notes" className="block text-sm text-brand-gray mb-1">
