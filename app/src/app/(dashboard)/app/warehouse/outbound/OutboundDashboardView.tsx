  ...
  if (loading) {
    return (
      <div className="py-2 font-serif">
        <h1 className="text-2xl text-brand-blue font-semibold mb-6">Outbound</h1>
  ...
  return (
    <div className="py-2 space-y-8 font-serif">
      ...
      <div className="flex items-center gap-2 text-sm text-brand-gray">
        ...
        <span>/</span>
        <span className="text-brand-black">Outbound</span>
      </div>

      <h1 className="text-2xl text-brand-blue font-semibold">Outbound</h1>

      {/* Summary cards */}
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-lg border border-brand-gray/20 shadow-md p-5">
          <p className="text-sm text-brand-gray">Upcoming Deliveries</p>
          <p className="text-2xl font-semibold text-brand-black">{summary.upcomingDeliveries}</p>
        </div>
        <div
          className={`rounded-lg border shadow-md p-5 ${
            summary.needsScheduling > 0
              ? "bg-yellow-50 border-yellow-300"
              : "bg-white border-brand-gray/20"
          }`}
        >
          <p
            className={`text-sm ${summary.needsScheduling > 0 ? "text-yellow-800" : "text-brand-gray"}`}
          >
            Needs Scheduling
          </p>
          <p
            className={`text-2xl font-semibold ${summary.needsScheduling > 0 ? "text-yellow-800" : "text-brand-black"}`}
          >
            {summary.needsScheduling}
          </p>
        </div>
        <div className="bg-white rounded-lg border border-brand-gray/20 shadow-md p-5">
          <p className="text-sm text-brand-gray">Active Transfers</p>
          <p className="text-2xl font-semibold text-brand-black">{summary.activeTransfers}</p>
        </div>
      </div>

      {/* Scheduled Deliveries */}
      <div>
        <h2 className="text-lg text-brand-black font-semibold mb-3">Scheduled Deliveries</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-brand-gray/20 bg-brand-stripe">
                <th className="text-left px-4 py-3 font-medium text-brand-gray">Date</th>
                <th className="text-left px-4 py-3 font-medium text-brand-gray">Time</th>
                ...
                <th className="text-left px-4 py-3 font-medium text-brand-gray">Zone</th>
            ...
            <tbody>
              {deliveries.length === 0 ? (
                ...
                deliveries.map((d) => (
                  <tr key={d.id} className="border-b border-brand-gray/10 hover:bg-brand-stripe/50">
                    <td className="px-4 py-2 text-brand-black">
                      {d.scheduledDate ? format(new Date(d.scheduledDate), "MMM d, yyyy") : "TBD"}
                    </td>
                    <td className="px-4 py-2 text-brand-gray">{d.scheduledTime || "--"}</td>
                    <td className="px-4 py-2 text-brand-gray">{d.customerName}</td>
                    ...
                    <td className="px-4 py-2 text-brand-gray">{d.zoneName || "--"}</td>
                    <td className="px-4 py-2">
                      <span
                        className={`text-xs px-2 py-0.5 rounded ${STATUS_STYLES[d.status] || "bg-brand-gray/20 text-brand-gray"}`}
                      >
      ...
      {/* Needs Scheduling */}
      <div>
        <h2 className="text-lg text-brand-black font-semibold mb-3">Needs Scheduling</h2>
        <div className="bg-white rounded-lg border border-brand-gray/20 shadow-md overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-brand-gray/20 bg-brand-stripe">
                <th className="text-left px-4 py-3 font-medium text-brand-gray">Order #</th>
                <th className="text-left px-4 py-3 font-medium text-brand-gray">Customer</th>
                <th className="text-left px-4 py-3 font-medium text-brand-gray">Order Date</th>
                <th className="text-right px-4 py-3 font-medium text-brand-gray">Items</th>
                <th className="text-left px-4 py-3 font-medium text-brand-gray">Days Waiting</th>
              </tr>
            </thead>
            <tbody>
              {needsScheduling.length === 0 ? (
                ...
                needsScheduling.map((order) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-right text-brand-gray">{order.lineItemCount}</td>
                    <td className="px-4 py-2">
                      <span
      ...
      {/* Stock Transfers */}
      <div>
        <h2 className="text-lg text-brand-black font-semibold mb-3">Stock Transfers</h2>
        <div className="bg-white rounded-lg border border-brand-gray/20 shadow-md overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-brand-gray/20 bg-brand-stripe">
                <th className="text-left px-4 py-3 font-medium text-brand-gray">From</th>
                <th className="text-left px-4 py-3 font-medium text-brand-gray">To</th>
                <th className="text-right px-4 py-3 font-medium text-brand-gray">Items</th>
                <th className="text-left px-4 py-3 font-medium text-brand-gray">Status</th>
                <th className="text-left px-4 py-3 font-medium text-brand-gray">Shipped</th>
              </tr>
            </thead>
            <tbody>
              {transfers.length === 0 ? (
                ...
                transfers.map((t) => (
                  <tr key={t.id} className="border-b border-brand-gray/10 hover:bg-brand-stripe/50">
                    <td className="px-4 py-2 text-brand-black">{t.fromLocation}</td>
                    <td className="px-4 py-2 text-brand-gray">{t.toLocation}</td>
                    <td className="px-4 py-2 text-right text-brand-gray">{t.itemCount}</td>
                    <td className="px-4 py-2">
                      <span
                        className={`text-xs px-2 py-0.5 rounded ${STATUS_STYLES[t.status] || "bg-brand-gray/20 text-brand-gray"}`}
                      >
