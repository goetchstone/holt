    ...
    <div className="py-2 space-y-6 font-serif">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-semibold text-brand-blue">Payments</h1>
        </div>
        <div className="flex gap-3">
          <Link
            href="/app/inventory/consignment/po-management"
            className="px-4 py-2 text-sm font-semibold border border-brand-navy text-brand-navy rounded-lg hover:bg-brand-linen transition min-h-[44px] flex items-center"
          >
            PO Management
          </Link>
          <Link
      ...
      {showCreate && (
        <div className="bg-white rounded-lg border border-brand-gray/20 shadow-md p-5 space-y-4">
          <h2 className="text-lg font-semibold text-brand-black">New Payment Batch</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label htmlFor="batch-period-start" className="block text-xs text-brand-gray mb-1">
                Period Start
              </label>
              <input
            ...
            <div>
              <label htmlFor="batch-period-end" className="block text-xs text-brand-gray mb-1">
                Period End
              </label>
              <input
                id="batch-period-end"
            ...
            <div>
              <label htmlFor="batch-check-number" className="block text-xs text-brand-gray mb-1">
                Check Number
              </label>
              <input
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-brand-gray/20 bg-brand-linen">
              <th className="text-left px-4 py-3 text-brand-gray font-semibold">Batch Date</th>
              <th className="text-left px-4 py-3 text-brand-gray font-semibold">Period</th>
              <th className="text-left px-4 py-3 text-brand-gray font-semibold">Check / PO #</th>
              <th className="text-right px-4 py-3 text-brand-gray font-semibold">Items</th>
              <th className="text-right px-4 py-3 text-brand-gray font-semibold">Total</th>
...
function BatchRow({ batch, striped, expanded, onToggle }: Readonly<BatchRowProps>) {
  ...
  return (
    <>
      ...
      >
        <td className="px-4 py-3 text-brand-black">{formatDate(batch.batchDate)}</td>
        ...
        <td className="px-4 py-3 text-brand-black">
          ...
          {batch.poNumber && !batch.purchaseOrderId && (
            <span className="ml-2 text-xs text-brand-gray">({batch.poNumber})</span>
          )}
        </td>
        <td className="px-4 py-3 text-brand-black text-right">{batch.itemCount}</td>
        <td className="px-4 py-3 text-brand-black text-right">{fmt(batch.totalAmount)}</td>
        <td className="px-4 py-3">
          <span className="inline-block px-2 py-0.5 rounded-full text-xs font-semibold bg-brand-gold/20 text-brand-gold">
            {batch.status}
          </span>
        </td>
      </tr>
      {expanded && batch.items && (
        <tr>
          <td colSpan={6} className="bg-brand-linen/50 px-6 py-4">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-brand-gray/20">
                  <th className="text-left px-3 py-2 text-brand-gray font-semibold">Barcode</th>
                  <th className="text-left px-3 py-2 text-brand-gray font-semibold">Quality</th>
                  <th className="text-left px-3 py-2 text-brand-gray font-semibold">Size</th>
                  <th className="text-right px-3 py-2 text-brand-gray font-semibold">Cost</th>
                  <th className="text-left px-3 py-2 text-brand-gray font-semibold">Sale Date</th>
                  <th className="text-left px-3 py-2 text-brand-gray font-semibold">Customer</th>
                </tr>
              </thead>
              <tbody>
                {batch.items.map((item) => (
                  <tr key={item.barcode} className="border-b border-brand-gray/10">
                    <td className="px-3 py-2 text-brand-black">{item.barcode}</td>
                    <td className="px-3 py-2 text-brand-black">{item.quality}</td>
                    <td className="px-3 py-2 text-brand-black">{item.size}</td>
                    <td className="px-3 py-2 text-brand-black text-right">{fmt(item.cost)}</td>
                    <td className="px-3 py-2 text-brand-black">{formatDate(item.saleDate)}</td>
                    <td className="px-3 py-2 text-brand-black">{item.saleCustomerName || "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
