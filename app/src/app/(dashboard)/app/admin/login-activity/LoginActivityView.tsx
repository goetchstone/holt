...
function StatusBadge({ row }: Readonly<{ row: LoginActivityRow }>) {
  ...
  return <span className="text-xs text-brand-gray">{formatLastSeen(row.lastSeenAt)}</span>;
}

function LastLoginCell({ row }: Readonly<{ row: LoginActivityRow }>) {
  if (!row.lastLoginAt) {
    return <span className="text-brand-gray/60 italic">never logged in</span>;
  }
  return (
...
export function LoginActivityView() {
  ...
  return (
    <div className="py-2 font-serif space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-brand-blue">Login Activity</h1>
      ...
      <div className="bg-white border border-brand-gray/20 rounded-lg shadow-sm p-4 flex items-center gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm">
            <span className="font-semibold text-brand-black">{activeCount}</span>{" "}
            <span className="text-brand-gray">active now</span>
      ...
      {loading && !data && <p className="text-brand-gray text-center py-8">Loading…</p>}
      {!loading && rows.length === 0 && (
        <p className="text-brand-gray text-center py-8">No staff to show.</p>
      )}
      {rows.length > 0 && (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-brand-linen border-b border-brand-gray/20">
                <th className="text-left p-3 font-semibold text-brand-black">Name</th>
                <th className="text-left p-3 font-semibold text-brand-black">Email</th>
                <th className="text-left p-3 font-semibold text-brand-black">Role</th>
                <th className="text-left p-3 font-semibold text-brand-black">Last Login</th>
            ...
            <tbody>
              {rows.map((row) => (
                <tr
                  key={row.id}
                  className={`border-b border-brand-gray/10 ${row.isActive ? "" : "opacity-60"} ${
                    isActiveNow(row.lastSeenAt) ? "bg-green-50/40" : "hover:bg-brand-stripe"
                  }`}
                >
                  <td className="p-3 text-brand-black font-medium">
                    {row.displayName}
                    {!row.isActive && (
                      <span className="ml-2 text-xs uppercase tracking-wide text-brand-gray">
                        inactive
                      </span>
                    )}
                  </td>
                  <td className="p-3 text-brand-gray text-xs">{row.email ?? "—"}</td>
                  <td className="p-3 text-brand-gray text-xs uppercase tracking-wide">
                    {row.role}
                  </td>
