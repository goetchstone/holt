...
function marginColor(pct: number | null): string {
  ...
  if (pct >= 25) return "text-brand-black";
  return "text-red-700";
}
  ...
  return (
    <div className="space-y-6 font-serif">
      <nav className="text-sm text-brand-gray">
        ...
        <span className="text-brand-black">Gross Margin</span>
      </nav>
      <h1 className="text-2xl font-semibold text-brand-navy">Gross Margin</h1>
      ...
      <div className="flex flex-wrap items-end gap-4">
        <div>
          <label htmlFor="gmStart" className="mb-1 block text-xs font-medium text-brand-gray">
            Start
          </label>
          <input
            id="gmStart"
        ...
        <div>
          <label htmlFor="gmEnd" className="mb-1 block text-xs font-medium text-brand-gray">
            End
          </label>
          <input
            id="gmEnd"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-brand-gray/20 bg-brand-linen">
                ...
                <th className="px-4 py-3 text-right font-semibold text-brand-gray">Cost</th>
            ...
            <tbody>
              {data.rows.map((row, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right text-brand-gray">{fmt(row.cost)}</td>
      ...
      {data && !loading && data.rows.length === 0 && (
        <p className="py-8 text-center text-brand-gray">No sales data for the selected period.</p>
      )}
