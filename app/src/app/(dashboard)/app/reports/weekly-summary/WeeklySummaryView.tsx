  ...
  return (
    <div className="mx-auto mt-8 max-w-6xl space-y-6 font-serif">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-brand-blue">Weekly Sales Summary</h1>
        ...
        <div className="flex items-end space-x-4">
          <div>
            <label htmlFor="weekStart" className="mb-1 block font-serif text-sm text-brand-black">
              Week (any day in it)
            </label>
            <input
              id="weekStart"
          ...
          <div>
            <label htmlFor="groupBy" className="mb-1 block font-serif text-sm text-brand-black">
              Group By
            </label>
            <select
              id="groupBy"
      ...
      {data?.traffic && (
        <div className="flex flex-wrap items-center gap-6 rounded-lg border bg-white p-4 shadow-sm">
          ...
          <div>
            ...
            <div className="text-xl font-semibold text-brand-gray">
              {data.traffic.totalLastYear.toLocaleString()}
            </div>
          </div>
          <div>
            <div className="text-xs uppercase tracking-wide text-brand-gray">Visitors vs LY</div>
      ...
      {data?.availableDepartments &&
        ...
        entityType !== "department" && (
          <div className="flex items-center gap-3">
            <span className="font-serif text-sm text-brand-black">Filter departments</span>
            <MultiSelectDropdown
              label="Departments"
            ...
            <table className="w-full border-collapse text-left text-sm">
              <thead>
                <tr className="bg-brand-linen">
                  ...
                  <th className="border-b-2 border-brand-gray p-2 text-right">This Week</th>
                  <th className="border-b-2 border-brand-gray p-2 text-right">Last Year</th>
                  <th className="border-b-2 border-brand-gray p-2 text-right">vs LY $</th>
                  <th className="border-b-2 border-brand-gray p-2 text-right">vs LY %</th>
                  {isCompany && (
                    <>
                      <th className="border-b-2 border-brand-gray p-2 text-right">Visitors</th>
                      <th
                        className="border-b-2 border-brand-gray p-2 text-right"
                        title="Sales transactions ÷ door visitors (whole store, ignores the department filter)"
                      >
                        Conv %
                      </th>
                      <th className="border-b-2 border-brand-gray p-2 text-right">Conv % LY</th>
                    </>
                  )}
                </tr>
              </thead>
              <tbody>
                {data.rows.map((r) => (
                  <tr key={r.entityName} className="odd:bg-white even:bg-gray-50">
                    ...
                    <td className="border-b border-brand-gray p-2 text-right text-brand-gray">
                      {currency(r.lastYear ?? 0)}
                    </td>
                    <td
                      className={`border-b border-brand-gray p-2 text-right ${signedClass(r.yoyVariance ?? 0)}`}
                    >
                      {currency(r.yoyVariance ?? 0)}
                    </td>
                    <td
                      className={`border-b border-brand-gray p-2 text-right ${signedClass(r.yoyPercent ?? 0)}`}
                    >
                      {pct(r.yoyPercent)}
                    </td>
                    {isCompany && (
                      <>
                        ...
                        <td className="border-b border-brand-gray p-2 text-right font-semibold">
              ...
              <tfoot>
                <tr className="border-t-2 border-brand-gray bg-brand-linen font-bold">
                  <td className="p-2">Totals</td>
                  <td className="p-2 text-right">{currency(totals.actual)}</td>
                  <td className="p-2 text-right text-brand-gray">{currency(totals.lastYear)}</td>
