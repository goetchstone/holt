  ...
  return (
    <div className="space-y-6 font-serif">
      <nav className="text-sm text-brand-gray">
        ...
        <span className="text-brand-black">Returns Analysis</span>
      </nav>
      <h1 className="text-2xl font-semibold text-brand-navy">Returns Analysis</h1>
      ...
      <div className="flex flex-wrap items-end gap-4">
        <div>
          <label htmlFor="rStart" className="mb-1 block text-xs font-medium text-brand-gray">
            Start
          </label>
          <input
            id="rStart"
        ...
        <div>
          <label htmlFor="rEnd" className="mb-1 block text-xs font-medium text-brand-gray">
            End
          </label>
          <input
            id="rEnd"
        ...
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="rounded-lg border border-brand-gray/20 bg-white p-4 shadow-sm">
              <div className="text-xs uppercase tracking-wide text-brand-gray">Gross sales</div>
              <div className="mt-1 text-2xl font-semibold text-brand-navy">
                {fmt(data.totals.grossSales)}
              </div>
            </div>
            <div className="rounded-lg border border-brand-gray/20 bg-white p-4 shadow-sm">
              <div className="text-xs uppercase tracking-wide text-brand-gray">Returns</div>
            ...
            <div className="rounded-lg border border-brand-gray/20 bg-white p-4 shadow-sm">
              <div className="text-xs uppercase tracking-wide text-brand-gray">Return rate</div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-brand-gray/20 bg-brand-linen">
                    ...
                    <th className="px-4 py-3 text-right font-semibold text-brand-gray">Returns</th>
                    <th className="px-4 py-3 text-right font-semibold text-brand-gray">
                      Return rate
                    </th>
                  </tr>
          ...
          {data.topReturnedProducts.length > 0 && (
            <div>
              <h2 className="mb-2 text-lg font-semibold text-brand-navy">Most returned products</h2>
              <div className="overflow-hidden rounded-lg border border-brand-gray/20 bg-white shadow-md">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-brand-gray/20 bg-brand-linen">
                      <th className="px-4 py-3 text-left font-semibold text-brand-gray">Product</th>
                      <th className="px-4 py-3 text-right font-semibold text-brand-gray">
                        Returns
                      </th>
                      <th className="px-4 py-3 text-right font-semibold text-brand-gray">Units</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.topReturnedProducts.map((p, i) => (
                      ...
                      >
                        <td className="px-4 py-3">
                          ...
                          {p.productNumber && (
                            <div className="text-xs text-brand-gray">{p.productNumber}</div>
                          )}
                        </td>
