...
export function ClientPortalView({
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-10 px-4 py-10 font-serif">
      <header>
        <p className="text-sm uppercase tracking-wide text-brand-gray">{appName}</p>
        <h1 className="text-2xl font-semibold text-brand-navy">Welcome, {data.customerName}</h1>
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-brand-navy">Appointments</h2>
        {upcoming.length === 0 && past.length === 0 && (
          <p className="text-sm text-brand-gray">No appointments yet.</p>
        )}
        {upcoming.length > 0 && (
          <ul className="space-y-2">
            {upcoming.map((a) => (
              <li
                key={a.id}
                className="flex items-center justify-between rounded-lg border border-brand-gray/20 bg-white px-4 py-3"
              >
                <div>
                  <div className="font-semibold text-brand-navy">
                    {a.serviceName ?? "Appointment"}
                  </div>
                  <div className="text-sm text-brand-gray">{fmtDateTime(a.startsAt, locale)}</div>
                </div>
                <span className="rounded-full bg-brand-stripe px-2 py-1 text-xs text-brand-gray">
                  {a.status}
                </span>
              </li>
            ))}
        ...
        {past.length > 0 && (
          <details className="mt-2">
            <summary className="cursor-pointer text-sm text-brand-gray">Past appointments</summary>
            <ul className="mt-2 space-y-1">
              {past.map((a) => (
                <li key={a.id} className="text-sm text-brand-gray">
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-brand-navy">Invoices</h2>
        {error && (
          <p className="mb-2 rounded border border-red-300 bg-red-50 px-3 py-2 text-sm text-red-700">
            {error}
          </p>
        )}
        {data.invoices.length === 0 && <p className="text-sm text-brand-gray">No invoices yet.</p>}
        <ul className="space-y-2">
          {data.invoices.map((inv) => (
            <li
              key={inv.id}
              className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-brand-gray/20 bg-white px-4 py-3"
            >
              <div>
                <div className="font-semibold text-brand-navy">
                  {inv.invoiceNo}{" "}
              ...
              {inv.status === "ISSUED" && inv.openBalance > 0 && (
                <button
                  ...
                  className="min-h-[44px] rounded-lg bg-brand-navy px-5 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-50"
                >
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-brand-navy">Support Requests</h2>
        {data.tickets.length === 0 && (
          <p className="text-sm text-brand-gray">No support requests yet.</p>
        )}
        <ul className="space-y-2">
          {data.tickets.map((t) => (
            <li
            ...
            >
              <div>
                ...
                <div className="text-sm text-brand-gray">{fmtDate(t.created, locale)}</div>
              </div>
              <span className="rounded-full bg-brand-stripe px-2 py-1 text-xs text-brand-gray">
                {t.status}
              </span>
      ...
      <footer className="border-t border-brand-gray/20 pt-4 text-xs text-brand-gray">
