// /app/src/pages/api/consignment/items/mark-sold.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { itemIds, saleDate, saleCustomerName, saleTransactionId, salesOrderId } = req.body;
  ...
  try {
    ...
    const invalid = items.filter((i) => i.status !== "ON_FLOOR" && i.status !== "ON_APPROVAL");
    if (invalid.length > 0) {
      return res.status(400).json({
        error: `Items must be ON_FLOOR or ON_APPROVAL to mark as sold`,
      ...
      itemIds.map((itemId: number) =>
        prisma.consignmentItem.update({
          ...
          data: {
            ...
            saleCustomerName: saleCustomerName || null,
    ...
    logError("Error marking consignment items as sold", error);
    return res.status(500).json({ error: "Failed to mark items as sold" });
  }
}
